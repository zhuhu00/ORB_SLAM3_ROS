...
/** 1. 检查SLAM系统传感器类型，对后续系统进行初始化
 *  2. 检查传感器配置文件，如相机标定参数之类的
 *  3. 实例化ORB词袋模型，并进行加载
 *  4. 根据ORB词袋模型，创建关键帧的库DataBase
 *  5. 创建多地图系统，即Altas
 *  6. IMU传感器的初始化设置，针对有IMU的系统,设置Map.cc中的mbIsInertial = true;
 *  7. 利用Atlas创建绘制器Drawer，其中包含FrameDrawer和MapDrawer
 *  8. 初始化Tracking线程
 *  9. 初始化局部地图LocalMapping线程，并进行加载
 *  10. 初始化闭环线程，LoopClosing，并加载
 *  11. 初始化用户初始化线程，并进行加载
 *  12. 将上述实例化的对象指针，传入需要的线程中
...
**/
    ...
    //获取字典加载状态,使用bin或者txt文件都可.
    bool bVocLoad;
    std::string suffixStrbVoc = strVocFile.substr(strVocFile.find_last_of('.') + 1);//获取文件后缀
    if(suffixStrbVoc=="txt"){
        std::cout<< "Load Vocabulary from Vocabulary."<<suffixStrbVoc<<endl;
        ...
        std::cout<< "Load Vocabulary."<<suffixStrbVoc<< " in "<< msElapsed<<"ms==="<<endl;

    }
    else if(suffixStrbVoc=="bin"){
        std::cout<< "Load Vocabulary from Vocabulary."<<suffixStrbVoc<<endl;
    ...
    // 在ORB-SLAM1,2中，这里是mpMap = new Map();不是多地图系统。
    //Create the Atlas
    mpAtlas = new Atlas(0);
...
/**
 * @brief: 双目跟踪的部分
 * @param {Mat} &imLeft 左边相机的图像
 * @param {Mat} &imRight 右边相机的图像
 * @param {double} &timestamp 图像时间戳
 * @param {string} filename 图像
 * @return {*}
 */
...
*/
    ...
    // 1. 检查SLAM系统的传感器类型，如果不是Stereo或者IMU_Stereo则退出。
    ...
    // Check mode change
    {
        ...
        if(mbActivateLocalizationMode)
        {
            ...
            // Tracking线程切换为只进行Tracking操作
            mpTracker->InformOnlyTracking(true);
            mbActivateLocalizationMode = false;
        }
        // 如果关闭了定位模式，则为同时定位和建图，默认是关闭定位模式
    ...
    // 3. 检查重置状态，也就是重置Tracking线程或者Activate Map，默认false
    // Check reset
    {
